...
pub fn divide(a: i32, b: i32) -> Result<i32, DivisionError> {
    ...
    if (divide_result * b - a) == 0 {
        return Ok(divide_result);
    }
...
mod tests {
    ...
    fn result_with_list() {
        ...
        let division_results: Vec<i32> = numbers.into_iter().map(|n| divide(n, 27)).map(|n| {
            match n {
                Result::Ok(i) => i,
                Err(_) => panic!("wrong"),
            }
        }).collect();
        assert_eq!(format!("{:?}", Result::<Vec<i32>, Vec<i32>>::Ok(division_results)), "Ok([1, 11, 1426, 3])");
    }
